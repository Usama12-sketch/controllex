/*
  Warnings:

  - Made the column `userId` on table `Archives` required. This step will fail if there are existing NULL values in that column.

*/
-- DropForeignKey
ALTER TABLE "Archives" DROP CONSTRAINT "Archives_userId_fkey";

-- AlterTable
ALTER TABLE "Archives" ALTER COLUMN "userId" SET NOT NULL;

-- AddForeignKey
ALTER TABLE "Archives" ADD CONSTRAINT "Archives_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
